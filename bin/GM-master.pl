      ...
      if ($LGETGRIB>0) {
        ...
        REGION: foreach $regionkey (@REGION_DOLIST) {
          ...
          if($GRIBFILE_MODEL eq 'ETA') {
            ...
            `rm -f Vtable; ln -s ../RUN.TABLES/Vtable.NAM Vtable`;  # It is only necessary to do this once!!
          }
          else {
            `link_grib.csh GRIB/gfs*` ;
            `rm -f Vtable; ln -s ../RUN.TABLES/Vtable.GFS Vtable`;  # It is only necessary to do this once!!
          }
            ...
            for( $IWINDOW=$iwindowstart{$regionkey}; $IWINDOW<= $iwindowend{$regionkey}; $IWINDOW++ ) {
              ...
              for($iline=0; $iline<=$#namelistlines; $iline++){
                ...
                if($line =~ m/opt_metgrid_tbl_path/i   ) {
                  $BaseDir = $ENV{"BASEDIR"};
                  $namelistlines[$iline] = sprintf(" opt_metgrid_tbl_path   = '%s/RUN.TABLES'\n", $BaseDir) ;
                }
              }
